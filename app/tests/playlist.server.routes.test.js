...
describe('Playlist CRUD tests', function() {
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/playlists')
					...
					.end(function(playlistSaveErr, playlistSaveRes) {
						...
						agent.put('/playlists/' + playlistSaveRes.body._id)
							.send(playlist)
							.expect(200)
	...
	it('should be able to get a single Playlist if not signed in', function(done) {
		...
		// Save the Playlist
		playlistObj.save(function() {
			request(app).get('/playlists/' + playlistObj._id)
				.end(function(req, res) {
					// Set assertion
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/playlists')
					...
					.end(function(playlistSaveErr, playlistSaveRes) {
						...
						agent.delete('/playlists/' + playlistSaveRes.body._id)
							.send(playlist)
							.expect(200)
	...
	it('should not be able to delete Playlist instance if not signed in', function(done) {
		...
		playlistObj.save(function() {
			...
			request(app).delete('/playlists/' + playlistObj._id)
			.expect(401)
			.end(function(playlistDeleteErr, playlistDeleteRes) {
				// Set message assertion
				(playlistDeleteRes.body.message).should.match('User is not logged in');
